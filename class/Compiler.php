...
class Compiler {
    ...
    public function prepare() {
        ...
        if (isset($match['params']['lang'])) {
            $lang_id = Languages::getIDFromABR($match['params']['lang']);
        }
        Languages::setLanguage($lang_id);

        $this->processSessionCleanup();
        $this->style = $this->processStyle($request);
        $this->_session = $this->processSession($request);
        ...
        if(!$this->bundles) {
            $this->bundles = $optimizer->addBundles(
                $styleid,
                $bundles,
                $this->compileBundles($bundles)
            );
        }
    ...
    public function processSessionCleanup() {
        /*
    ...
    public function processStyle(&$request) {
    //      global $usersSupport;
        $styles = $this->getStyles();
        $styleid = isset($_COOKIE[PhpcStyleCookie])
            ? (int)$_COOKIE[PhpcStyleCookie] : 0;

        //TODO support user's styles
        if(class_exists("UsersSupport")) {
            $usersSupport->processStyle($styleid);
        }
        ...
        if($bestWeight<0) Services::fatalError('fatal_nostyle');
    ...
    private function getStyles() {
        if (!isset($this->_cache)) {
            $this->_cache = array(); // arrangeArraysSet($database->getOrderedLines("styles","folder,id"));
        } 
        return $this->_cache;
    }
}
